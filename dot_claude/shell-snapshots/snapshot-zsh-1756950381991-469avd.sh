...
..znap.auto-compile () {
	# undefined
	builtin autoload -XUz /Users/datdo/Git/zsh-snap/functions
}
..znap.compinit-hook () {
	# undefined
	builtin autoload -XUz /Users/datdo/Git/zsh-snap/functions
...
..znap.dirname.n () {
	...
	..znap.repos-dir
	case $1 in
		(${1:t}) dirs=($gitdir/{*/,}$1/.git(Non-/:h))  ;;
		(${1:t2}) dirs=($gitdir/$1/.git(Non-/:h))  ;;
	esac
	(( $#dirs[@] == 1 )) || return
	reply=($dirs) 
}
...
..znap.repos () {
	...
	then
		...
		local -a exclude=() 
		if zstyle -a ":znap:$1:" exclude exclude
		then
...
..znap.repos-dir () {
	zstyle -s :znap: repos-dir gitdir || gitdir=${${(%):-%x}:P:h:h:a:h} 
}
..znap.tput () {
...
.znap.clone () {
	...
	for repo in $@
	do
		if [[ $repo != ([^/]##/[^/]##|*(@|://)*/*.git) ]] && ! [[ -d ${repo#file://} && $repo == */.git ]]
		then
	...
	local server='' 
	zstyle -s :znap:clone: default-server server || server='https://github.com/' 
	.znap.multi ".znap.clone.task $server "$^@ || return 'sysexits[(i)IOERR] + 63'
}
.znap.clone.task () {
	# undefined
	builtin autoload -XUz /Users/datdo/Git/zsh-snap/functions
...
.znap.eval () {
	...
	[[ -r $_P__cache_file ]] || print -r "znap eval: generating cache for ${(q+)_P__cmd}"
	private _P__line= _P__header="#${(q+)_P__cmd}" _P__index=$_P__repo/.git/index 
	(
		[[ -r $_P__cache_file ]] && IFS='' read -r _P__line < $_P__cache_file
		if [[ $_P__line != $_P__header ]] || [[ -n $_P__repo && -f $_P__index && $_P__index -nt $_P__cache_file ]]
		then
			[[ -d $_P__repo ]] && builtin cd -q -- $_P__repo
...
.znap.prompt () {
	...
	exec {_znap_prompt_fd}>&2
	exec 2> $_znap_prompt_tmp
	promptinit
	if [[ -n $theme ]]
	then
		prompt_themes+=($theme) 
...
.znap.prompt.load () {
	...
	[[ -v functions[$setup] ]] && return
	autoload +X -Uz $setup &> /dev/null && return
	unfunction $setup
	local repos=() 
	local gitdir= 
	...
	if [[ -n $file ]]
	then
		if autoload +X -Uz $file &> /dev/null
		then
			fpath=($file:h $fpath[@]) 
			return
		fi
...
.znap.source () {
	...
	for _P__src in ${@:-''}
	do
		[[ -h $XDG_STATE_HOME/${_P__repo:t2}${_P__src:+.}${_P__src//\//.} ]] && zf_rm -fr $XDG_STATE_HOME/${_P__repo:t2:h}
		_P__file=$_P__state/${_P__repo:t2}${_P__src:+.}${_P__src//\//.} 
		if ! [[ -r $_P__file:P ]]
		then
			.znap.source.link "$_P__repo" "$_P__src" "$_P__file" || return
		fi
...
prompt_pure_precmd () {
	functions[prompt_pure_precmd]=$functions[:znap:prompt_pure_precmd] 
	unfunction :znap:prompt_pure_precmd
}
prompt_pure_preexec () {
...
znap () {
	...
	if [[ -v functions[$_P__func] ]]
	then
		if (( ${#opts_pre} || ${#opts_post} ))
		then
			.znap.help $_P__cmd
		else
			$_P__func $@
...
alias -- copyssh='pbcopy < /Users/datdo/.ssh/id_ed25519.pub'
alias -- diff='git diff'
...
alias -- l='ls -lah'
alias -- la='ls -lAh'
alias -- library='cd /Users/datdo/Library'
...
alias -- projects='cd /Users/datdo/workspace'
...
alias -- sites='cd /Users/datdo/Herd'
...
alias -- workspace='cd /Users/datdo/workspace'
# Check for rg availability
if ! command -v rg >/dev/null 2>&1; then
  alias rg=''\''/Users/datdo/Library/Application Support/Zed/external_agents/claude-code-acp/0.2.5/node_modules/@anthropic-ai/claude-code/vendor/ripgrep/arm64-darwin/rg'\'''
fi
export PATH='/Users/datdo/bin:/usr/local/bin:/Users/datdo/workspace/agent/.venv/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/System/Cryptexes/App/usr/bin:/usr/bin:/bin:/usr/sbin:/sbin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin:/Users/datdo/.local/share/pnpm:/Users/datdo/.local/bin:/Users/datdo/.local/share/chezmoi/helpers:/Applications/Visual Studio Code.app/Contents/Resources/app/bin:/Users/datdo/.customscripts:/Users/datdo/.cargo/bin:/Users/datdo/.orbstack/bin'
