...
struct Environment {
	...
	static let isInferenceActionCopiedCompletion: Bool = CommandLine.arguments.contains("--inference-action-copied")
...
struct HUDView: View {
	...
	let width: CGFloat = Environment.isInferenceActionCopiedCompletion
		? 220.0
		: Environment.text.width(font: NSFont.systemFont(ofSize: 13.0))
	...
	var body: some View {
		ZStack {
			if isShowing {
				Background()
				if !Environment.isInferenceActionCopiedCompletion {
					DisplayText()
				} else {
					InferenceActionCopyCompletedText()
				}
			}
		}
	...
	@ViewBuilder
	func InferenceActionCopyCompletedText() -> some View {
		Text("􀅴 Result copied to clipboard")
			.font(.system(size: 13.0))
			.foregroundColor(.white)
			.padding([.leading, .trailing], 4)
			.frame(alignment: .center)
	...
	func applicationDidFinishLaunching(_ aNotification: Notification) {
		...
		if let screenFrame: NSRect = NSScreen.main?.visibleFrame {
			if #available(macOS 15, *) {
				let midYShift: CGFloat = Environment.isInferenceActionCopiedCompletion ? -0.2 : 0.86
				let windowFrame = window.frame
				let x = screenFrame.midX - windowFrame.width / 1.75
				let y = screenFrame.midY - (screenFrame.midY * midYShift)
				window.setFrameOrigin(NSPoint(x: x, y: y))
			} else {
				let midYShift: CGFloat = Environment.isInferenceActionCopiedCompletion ? -0.2 : 0.825
				let windowFrame = window.frame
